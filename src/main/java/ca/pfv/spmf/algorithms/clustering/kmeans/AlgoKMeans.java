...
public class AlgoKMeans {
	 ...
	 */
	public List<Cluster> runAlgorithm(String inputFile, int k, DistanceFunction distanceFunction) throws NumberFormatException, IOException {
	 ...
	 * @param vectorsSize  the vector size
	 */
	List<Cluster> applyKMeans(int k, DistanceFunction distanceFunction,
			List<DoubleArray> vectors, double minValue, double maxValue,
			int vectorsSize) {
	...
	double getSSE(List<Cluster> clusters) {
		double sse = 0;
